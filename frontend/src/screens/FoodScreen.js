...
function FoodScreen() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        const result = await axios.get(`/api/foods/slug/${slug}`);
  ...
  useEffect(() => {
    const fecthRelatedFoods = async () => {
      dispatch({ type: "FETCH_RELATED_REQUEST" });
      try {
        const result = await axios.get(`/api/foods/${food._id}/related`);
    ...
    // Call the fecthRelatedFoods function if product._id is available
    if (food._id) {
      fecthRelatedFoods();
    }
  }, [food._id]);
  ...
  const submitHandler = async (e) => {
    ...
    try {
      ...
      const updatedFood = {
        ...food,
        reviews: [data.review, ...food.reviews],
    ...
    if (window.confirm("Are you sure you want to delete this review?")) {
      try {
        const { data } = await axios.delete(
          `/api/foods/${food._id}/reviews/${reviewId}`,
          {
            headers: { Authorization: `Bearer ${userInfo.token}` },
          }
        );
  ...
  const handleEditReview = async (e) => {
    ...
    try {
      ...
      const { data } = await axios.put(
        `/api/foods/${food._id}/reviews/${editReview._id}`,
        {
          rating,
          comment,
        },
  ...
  const renderEditFood = () => {
    if (userInfo && userInfo.isAdmin) {
      return (
        <Link to={`/admin/food/${food._id}`}>
          <Button variant="secondary" className="my-3">
            <div className="fas fa-edit"></div> Edit Food{" "}
  ...
  return loading ? (
    ...
    <div style={{ marginTop: "10px" }}>
      <Row>
          ...
          {selectedImg && (
            <Card className="mb-2 appear-animation">
              <img
                ...
                src={selectedImg || food.img}
                alt={food.name}
                className="img-product"
                onClick={() => handleImageClick(selectedImg || food.img)}
              />
            </Card>
          )}
        </Col>
        ...
        <Col md={3}>
          <ListGroup variant="flush">
            ...
            <ListGroup.Item>
              <strong>
                <div className="fas fa-coins"></div> Food Sold:{" "}
              </strong>
              {food.sold}
            </ListGroup.Item>
            <ListGroup.Item>
          ...
          <Card>
            <Card.Body>
              <ListGroup variant="flush">
                ...
                <ListGroup.Item>{renderEditFood()}</ListGroup.Item>
              </ListGroup>
            </Card.Body>
          </Card>
        </Col>
      ...
      {loadingRelated ? (
        ...
        <div className="info">No related foods available.</div>
      ) : (
        <div>
          <h1>
            <Badge bg="info">Related Foods</Badge>
          </h1>

          <Row xs={1} md={6} className="g-4">
            {relatedFoods.map((relatedFood) => (
              <Col key={relatedFood._id}>
                <Card className="thumbnail-card appear-animation">
                  <Link to={`/food/${relatedFood.slug}`}>
                    <OverlayTrigger
                      placement="bottom"
                      overlay={
                        <Tooltip id="tooltip">{relatedFood.name}</Tooltip>
                      }
                    >
                      <Card.Img
                        variant="top"
      ...
      <div className="my-3">
        ...
        <Card style={{ padding: "10px" }}>
          ...
          <div style={{ marginBottom: "15px" }}>
            {food.description.length > 500 ? (
              <>
                {showFullDescription ? (
                  <div
                    dangerouslySetInnerHTML={{
                      __html: DOMPurify.sanitize(food.description),
                    }}
                  />
                ) : (
                  <div
          ...
          {userInfo ? (
            ...
            <div className="warning">
              ...
              <Link
                ...
                to={`/signin?redirect=/food/${food.slug}`}
              >
                Sign In
              </Link>{" "}
              to write a review
      ...
      <Modal show={showImageModal} onHide={() => setShowImageModal(false)}>
        ...
        <Modal.Body>
          <OverlayTrigger
            placement="bottom"
            overlay={<Tooltip id="tooltip">{food.name}</Tooltip>}
          >
            <img
              src={selectedImg}
              alt={food.name}
